...
open class AMPFloatingTextField: UITextField { // swiftlint:disable:this type_body_length
    ...
    @IBInspectable dynamic open var errorColor: UIColor = .black {
        didSet {
            updateColors()
        }
    }
    ...
    open func updateBorder() {
        ...
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiom.pad) {
            ...
            borderLayer.frame = CGRect( x: bounds.origin.x, y: 20, width: UIScreen.main.bounds.size.width-60, height: 55)
        }
        else {
            
            borderLayer.frame = CGRect( x: bounds.origin.x, y: titleHeight(), width: UIScreen.main.bounds.size.width-60, height: bounds.size.height - titleHeight() * 2 - lineHeight)
        }
...
//        errorLabel.font = titleFont
    ...
    override open func placeholderRect(forBounds bounds: CGRect) -> CGRect {
        let rect = CGRect(x: 4, y: titleHeight(), width: bounds.size.width, height: bounds.size.height - titleHeight() * 2 - lineHeight
        )
        return rect
    }
    ...
    open func errorLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        ...
        let height = bounds.size.height - titleHeight()
